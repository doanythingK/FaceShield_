...
public unsafe sealed class VideoExportService
{
        ...
        VideoHdrMetadata? hdrMetadata)
    {
        ...
        foreach (var candidateName in candidateNames)
        {
            ...
            AVChromaLocation sourceChromaLocation = VideoHdrMetadataPolicy.ResolveSourceChromaLocation(inStream, dec);
        ...
        VideoHdrMetadata? hdrMetadata)
    {
        ...
        ctx->chroma_sample_location = VideoHdrMetadataPolicy.ResolveSourceChromaLocation(inStream, dec);
        if (isLosslessX264Rgb)
        {
        ...
        if (hdrMetadata?.HasStaticMetadata == true)
        {
            ...
            if (isSvtAv1 &&
                !VideoHdrMetadataPolicy.CanConfigureSvtAv1StaticHdr(hdrMetadata, out string? hdrCompatibilityError))
            {
            ...
            if (isSvtAv1 &&
                !VideoHdrMetadataPolicy.TryConfigureEncoderStaticHdrMetadata(ctx, hdrMetadata, out string? hdrError))
            {
                error = hdrError;
                ffmpeg.avcodec_free_context(&ctx);
                return null;
