...
- Post-scene cleanup now also purges residual YOLO carry masks after known cut pairs when the post-cut box still matches the pre-cut position and remains at or below `0.90` confidence. For adjacent cut pairs it scans the normal 5-frame carry window. For direct source-to-target cut pairs it starts at `source+1` and continues through the confirmed target plus the carry window, so transition frames between `source` and `target` cannot keep an old blur. This is implemented in `YoloFinalMaskPostProcessor.RemoveSceneCutCarryRemnants` and shared by the GUI path and smoke harness. Removed carry frames are passed into the final gap-fill blocked-frame list, so the anti-flicker pass cannot recreate the same scene-transition residue.
