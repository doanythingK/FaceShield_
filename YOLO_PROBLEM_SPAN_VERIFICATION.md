...
- 컷 이후 final gap fill이 `blockedByCut=...`/`cutBlockedFrames=...` 또는 `blockedBySceneCarry=...`/`sceneCarryBlockedFrames=...`를 남기면, 후속 anti-flicker fill이 확인된 전환 구간의 잔상을 다시 만들지 않았다는 근거로 본다. scene-carry blocked frame이 stable gap 안에 하나라도 있으면 해당 gap 전체를 채우지 않는다. `blockedByCleanup=...`/`cleanupBlockedFrames=...`는 cleanup이 제거한 같은 얼굴 위치를 재생성하지 않았다는 근거다.
