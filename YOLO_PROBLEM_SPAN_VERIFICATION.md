...
- Post-scene cleanup now also purges residual YOLO carry masks for up to 5 frames after known cut pairs when the post-cut box still matches the pre-cut position and remains at or below `0.90` confidence. This is implemented in `YoloFinalMaskPostProcessor.RemoveSceneCutCarryRemnants` and shared by the GUI path and smoke harness. Removed carry frames are passed into the final gap-fill blocked-frame list, so the anti-flicker pass cannot recreate the same scene-transition residue.
- Deterministic verifier evidence: `scripts/verify-yolo-final-mask-cleanup.ps1` now includes `sceneCutCarryRemoved=5`, `sceneCutCarryFrames=1001,1002,1003,1004,1005`, and `sceneCutCarryRefillBlocked=5`.
