...
- Post-scene cleanup now also purges residual YOLO carry masks for up to 5 frames after known cut pairs when the post-cut box still matches the pre-cut position and remains at or below `0.90` confidence. Removed carry frames are passed into the final gap-fill blocked-frame list, so the anti-flicker pass cannot recreate the same scene-transition residue.
