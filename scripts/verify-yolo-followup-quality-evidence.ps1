...
Assert-Contains "script supports no-detection person object pseudo gt" $scriptText '\$detectionRows\.Count\s+-eq\s+0[\s\S]*WithPseudoGtPersonObjectInput[\s\S]*pseudo-GT no-detection person/object input'
...
Assert-Contains "script parses final mask post-gap-fill cleanup" $scriptText "SmokeYoloFinalMaskPostGapFillCleanup|YoloFinalMaskPostGapFillCleanup"
...
Assert-Contains "script forwards continuity candidates to pseudo gt evidence" $scriptText 'pseudoGtArgs[\s\S]*"-ContinuityCandidateCsv"[\s\S]*resolvedMaskContinuityCsvPath'
...
Assert-Contains "script passes continuity candidates to full-frame review package" $scriptText "ContinuityCandidateCsv[\s\S]*resolvedMaskContinuityCsvPath"
...
Assert-Contains "continuity reports tiny weak masks" $continuityText "Tiny weak final masks: 1"
Assert-Contains "continuity reports tiny short masks" $continuityText "Tiny short final masks: 1"
Assert-Contains "continuity reports protected scene carry frames" $continuityText "Protected scene-carry frames: 2"
Assert-Contains "continuity reports final summary review reasons" $continuityText "Final summary review reasons: ``short-gap,per-face-short-gap,large-jump-gap,isolated-mask,low-confidence-review,weak-non-edge,edge-weak-review,top-edge-weak-review,upper-weak,lower-weak,aspect-outlier,tiny-weak,tiny-short,scene-carry-protected``"
...
Assert-Contains "summary records final mask post-gap-fill cleanup" $summaryText "Final mask post-gap-fill cleanup[\s\S]*removedFrames=21"
...
Assert-Contains "summary records per-face gap summary" $summaryText "perFaceShortGaps=1"
...
Assert-Contains "summary records final mask review reasons" $summaryText "reviewRequired=True[\s\S]*reviewReasons=short-gap,per-face-short-gap,large-jump-gap,isolated-mask,low-confidence-review,weak-non-edge,edge-weak-review,top-edge-weak-review,upper-weak,lower-weak,aspect-outlier,tiny-weak,tiny-short,scene-carry-protected"
