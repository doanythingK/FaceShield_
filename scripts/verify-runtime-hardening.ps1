...
Assert-Match "session readiness disables playback before adoption" $sessionCoordinator '_frameList\.SetPlaybackEnabled\(false\)[\s\S]*AdoptSession\('
Assert-Match "session readiness keeps UI gated when adoption fails" $sessionCoordinator 'AdoptSession\([\s\S]*if\s*\(!adopted\)[\s\S]{0,260}_frameList\.SetPlaybackEnabled\(false\);[\s\S]{0,180}_framePreview\.SetSessionReady\(false\);[\s\S]{0,120}return;'
Assert-Match "session readiness enables UI only after successful adoption" $sessionCoordinator 'if\s*\(!adopted\)[\s\S]{0,500}return;[\s\S]{0,260}_frameList\.SetPlaybackEnabled\(true\);[\s\S]{0,180}_framePreview\.SetSessionReady\(true\);'
...
Assert-Match "manual load shutdown waits the full operation" $framePreview 'TaskCompletionSource<bool>\([\s\S]{0,220}operationTask[\s\S]{0,180}_manualFrameLoadTask[\s\S]*operationCompletion\.TrySetResult\(true\)'
Assert-Match "manual apply has explicit bitmap ownership transfer" $framePreview 'ApplyManualFrame\([\s\S]{0,180}out\s+frameOwnershipTransferred[\s\S]{0,220}!frameOwnershipTransferred[\s\S]{0,100}bitmap\.Dispose\(\)'
Assert-Match "manual playback disposes and invalidates uncommitted decoded frame" $framePreview 'RunManualSequentialPlaybackAsync\([\s\S]*if\s*\(!frameCommittedToPreview\)[\s\S]{0,180}frame\?\.Dispose\(\);[\s\S]{0,180}TryInvalidateManualSequentialPosition\(player\)'
Assert-Match "general playback disposes uncommitted decoded frame" $framePreview 'RunSequentialPlaybackAsync\([\s\S]*finally[\s\S]{0,160}!frameCommittedToPreview[\s\S]{0,120}frame\?\.Dispose\(\)'
Assert-Match "playback ownership transfer is marked at FrameBitmap assignment" $framePreview 'FrameBitmap\s*=\s*exact;[\s\S]{0,100}ownershipTransferred\s*=\s*true'
