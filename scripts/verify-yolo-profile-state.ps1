...
Assert-Match "final mask cleanup removes medium-confidence tiny isolated non-edge masks" $finalMaskPostProcessorText "confidence\s*<=\s*options\.TinyIsolatedMaxConfidence[\s\S]*IsTinyFace\(face,\s*data\.Size,\s*options\.TinyIsolatedMaxAreaRatio\)[\s\S]*!HasMatchingTemporalNeighbor"
