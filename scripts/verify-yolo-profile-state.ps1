...
Assert-Match "workspace passes yolo cut pairs to final gap fill" $workspaceText "postSmoothGuard\.CutFramePairs[\s\S]*RemoveYoloWeakIsolatedFinalMasks\([\s\S]*blockedCutFramePairs:\s*CombineCutFramePairs\(yoloPreSmoothCutPairs,\s*yoloPostSmoothCutPairs,\s*yoloCleanupPass\.CutFramePairs\)[\s\S]*bool\s+fillStableGaps\s*=\s*true[\s\S]*blockedCutFramePairs"
...
Assert-Match "workspace carries cleanup removals into post-scene final gap fill" $workspaceText "yoloCleanupPass\s*=\s*RemoveYoloWeakIsolatedFinalMasks\(FrameList\.VideoPath,\s*token\)[\s\S]*additionalBlockedFrameIndices:\s*yoloCleanupPass\.RemovedFrameIndices[\s\S]*CombineFrameIndices\(additionalBlockedFrameIndices,\s*cleanup\.RemovedFrameIndices\)"
Assert-Match "workspace carries initial gap-fill cut pairs into post-scene final gap fill" $workspaceText "CombineCutFramePairs\(yoloPreSmoothCutPairs,\s*yoloPostSmoothCutPairs,\s*yoloCleanupPass\.CutFramePairs\)[\s\S]*var\s+gapFillCutPairs[\s\S]*gapFillCutPairs\s*=\s*FillYoloStableFinalMaskGaps[\s\S]*return\s+new\s+YoloFinalMaskCleanupPassResult\(cleanup\.RemovedFrameIndices,\s*gapFillCutPairs\)[\s\S]*YoloFinalMaskCleanupPassResult[\s\S]*CutFramePairs"
