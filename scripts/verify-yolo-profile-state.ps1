...
Assert-Match "workspace passes yolo cut pairs to final gap fill" $workspaceText "postSmoothGuard\.CutFramePairs[\s\S]*ProbeYoloStrongCarrySceneCuts[\s\S]*var\s+yoloCutPairs\s*=\s*CombineCutFramePairs\([\s\S]*yoloPreSmoothCutPairs[\s\S]*yoloPostSmoothCutPairs[\s\S]*yoloCleanupPass\.CutFramePairs[\s\S]*yoloStrongCarryProbeCutPairs[\s\S]*RemoveYoloWeakIsolatedFinalMasks\([\s\S]*blockedCutFramePairs:\s*yoloCutPairs[\s\S]*bool\s+fillStableGaps\s*=\s*true[\s\S]*blockedCutFramePairs"
...
Assert-Match "workspace carries initial gap-fill cut pairs into post-scene final gap fill" $workspaceText "var\s+yoloCutPairs\s*=\s*CombineCutFramePairs\([\s\S]*yoloPreSmoothCutPairs[\s\S]*yoloPostSmoothCutPairs[\s\S]*yoloCleanupPass\.CutFramePairs[\s\S]*yoloStrongCarryProbeCutPairs[\s\S]*var\s+gapFillCutPairs[\s\S]*gapFillCutPairs\s*=\s*FillYoloStableFinalMaskGaps[\s\S]*return\s+new\s+YoloFinalMaskCleanupPassResult\(cleanup\.RemovedFrameIndices,\s*cleanup\.RemovedFacesInfo,\s*gapFillCutPairs\)[\s\S]*YoloFinalMaskCleanupPassResult[\s\S]*RemovedFacesInfo[\s\S]*CutFramePairs"
...
Assert-Match "workspace probes strong yolo scene carry without immediate removal" $workspaceText "YoloSceneCutStrongCarryProbeMaxConfidence\s*=\s*0\.995f[\s\S]*YoloSceneCutStrongCarryProbeMinConfidence\s*=\s*0\.78f[\s\S]*YoloSceneCutStrongCarryProbeMinSourceConfidence\s*=\s*0\.80f[\s\S]*ProbeYoloStrongCarrySceneCuts[\s\S]*BuildWeakPostCutCarryCandidates\([\s\S]*maxTargetConfidence:\s*YoloSceneCutStrongCarryProbeMaxConfidence[\s\S]*minSourceConfidence:\s*YoloSceneCutStrongCarryProbeMinSourceConfidence[\s\S]*minTargetConfidence:\s*YoloSceneCutStrongCarryProbeMinConfidence[\s\S]*removeCandidates:\s*false[\s\S]*YoloStrongCarrySceneCutProbe"
