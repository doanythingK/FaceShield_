...
Assert-Match "workspace carries cleanup removals into post-scene final gap fill" $workspaceText "yoloCleanupPass\s*=\s*RemoveYoloWeakIsolatedFinalMasks\(FrameList\.VideoPath,\s*token\)[\s\S]*RemoveSceneCutCarryRemnants\([\s\S]*_maskProvider[\s\S]*yoloCutPairs[\s\S]*additionalBlockedFrameIndices:\s*CombineFrameIndices\([\s\S]*yoloCleanupPass\.RemovedFrameIndices[\s\S]*yoloCarryCleanup\.RemovedFrameIndices[\s\S]*CombineFrameIndices\(additionalBlockedFrameIndices,\s*cleanup\.RemovedFrameIndices\)"
...
Assert-Match "final mask cleanup verifies scene-cut carry cleanup behavior" $finalMaskCleanupVerifyText "RemoveSceneCutCarryRemnants[\s\S]*Expected scene-cut carry cleanup to remove weak frames 1001-1005[\s\S]*Expected scene-cut carry cleanup frames to block final gap refill"
...
Assert-Match "smoke harness cleans weak remnants after scene guard" $smokeHarnessText "SmokeFaceTrackSceneCutGuard[\s\S]*RemoveSceneCutCarryRemnants[\s\S]*SmokeYoloSceneCutCarryCleanup[\s\S]*postProcessor\.RemoveWeakIsolatedMasks\(maskProvider\)[\s\S]*SmokeYoloFinalMaskPostSceneCleanup"
