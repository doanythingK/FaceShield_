...
Assert-Match "workspace logs yolo final cleanup reasons" $workspaceText "removedWeakUnsupported=\{cleanup\.RemovedWeakUnsupportedFaces\}[\s\S]*removedWeakShortClusters=\{cleanup\.RemovedWeakShortClusterFaces\}[\s\S]*removedWeakTinyClusters=\{cleanup\.RemovedWeakTinyClusterFaces\}"
Assert-Match "workspace fills stable yolo final gaps" $workspaceText "FillYoloStableFinalMaskGaps[\s\S]*FillShortStableGaps\(_maskProvider\)[\s\S]*YoloFinalMaskGapFill"
Assert-Match "workspace guards stable yolo final gaps across scene cuts" $workspaceText "FillYoloStableFinalMaskGaps[\s\S]*gapFill\.FilledFacesInfo[\s\S]*FaceTrackSceneCutGuard\(\)\.Apply[\s\S]*YoloFinalMaskGapFillSceneCutGuard"
Assert-Match "workspace broadens yolo scene cut carry candidates conservatively" $workspaceText "YoloSceneCutDirectCarryMaxConfidence\s*=\s*0\.72f[\s\S]*YoloSceneCutPostCutCarryMaxConfidence\s*=\s*0\.58f[\s\S]*BuildWeakTrackTransitionCandidates[\s\S]*maxTargetConfidence:\s*YoloSceneCutDirectCarryMaxConfidence[\s\S]*BuildWeakPostCutCarryCandidates[\s\S]*maxTargetConfidence:\s*YoloSceneCutPostCutCarryMaxConfidence"
...
Assert-Match "final mask cleanup fills stable strong gaps" $finalMaskPostProcessorText "FillShortStableGaps[\s\S]*MaxGapFrames[\s\S]*MinAnchorConfidence[\s\S]*TryFindStableGapMatch[\s\S]*SetFaceRects[\s\S]*FilledFacesInfo"
...
Assert-Match "workspace applies yolo scene cut before roi refine" $workspaceText "var\s+trackPost\s*=\s*ApplyAutoTemporalFixes\(\);[\s\S]*RemoveYoloTrackFillAcrossSceneCuts\(FrameList\.VideoPath,\s*trackPost,\s*token\);[\s\S]*RefineAutoFacesWithRoi"
Assert-Match "workspace applies yolo scene cut before temporal smoothing" $workspaceText "var\s+trackPost\s*=\s*ApplyAutoTemporalFixes\(\);[\s\S]*RemoveYoloTrackFillAcrossSceneCuts\(FrameList\.VideoPath,\s*trackPost,\s*token\);[\s\S]*ApplyAutoTemporalSmoothing\(\);"
...
Assert-Match "smoke harness logs final yolo mask summary" $smokeHarnessText "if\s*\(useYolo\)[\s\S]*LogFinalMaskSummary\(label,\s*maskProvider\)[\s\S]*SmokeFinalMaskSummary[\s\S]*shortGaps=\{shortGapCount\}[\s\S]*largeJumpGaps=\{largeJumpGapRanges\.Count\}[\s\S]*largeJumpRanges=\{FormatTextValues\(largeJumpGapRanges\)\}[\s\S]*isolatedFrames=\{FormatFrames\(isolatedFrames\)\}[\s\S]*weakNonEdge=\{weakNonEdgeRows\}[\s\S]*tinyWeak=\{tinyWeakRows\}"
...
Assert-Match "smoke harness logs yolo final cleanup reasons" $smokeHarnessText "removedWeakUnsupported=\{cleanup\.RemovedWeakUnsupportedFaces\}[\s\S]*removedWeakShortClusters=\{cleanup\.RemovedWeakShortClusterFaces\}[\s\S]*removedWeakTinyClusters=\{cleanup\.RemovedWeakTinyClusterFaces\}"
Assert-Match "smoke harness logs yolo final gap fill" $smokeHarnessText "FillShortStableGaps\(maskProvider\)[\s\S]*SmokeYoloFinalMaskGapFill"
Assert-Match "smoke harness guards yolo final gap fill across scene cuts" $smokeHarnessText "FillShortStableGaps\(maskProvider\)[\s\S]*gapFill\.FilledFacesInfo[\s\S]*FaceTrackSceneCutGuard\(\)\.Apply[\s\S]*SmokeYoloFinalMaskGapFillSceneCutGuard"
