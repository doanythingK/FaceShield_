...
Assert-Match "scene cut guard builds weak post-cut carry candidates" $sceneCutGuardText "BuildWeakPostCutCarryCandidates[\s\S]*maxCarryFrames[\s\S]*sourceLookbackFrames[\s\S]*BuildWeakCarryRun[\s\S]*HasStrongContinuation[\s\S]*lookbackFrames[\s\S]*sourceFrame[\s\S]*FaceTrackFilledFace"
