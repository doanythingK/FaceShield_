...
Assert-Match "workspace passes yolo cut pairs to final gap fill" $workspaceText "postSmoothGuard\.CutFramePairs[\s\S]*var\s+yoloCutPairs\s*=\s*CombineCutFramePairs\([\s\S]*yoloPreSmoothCutPairs[\s\S]*yoloPostSmoothCutPairs[\s\S]*yoloCleanupPass\.CutFramePairs[\s\S]*RemoveYoloWeakIsolatedFinalMasks\([\s\S]*blockedCutFramePairs:\s*yoloCutPairs[\s\S]*bool\s+fillStableGaps\s*=\s*true[\s\S]*blockedCutFramePairs"
Assert-Match "workspace fills stable yolo final gaps" $workspaceText "YoloFinalMaskStableGapMaxFrames\s*=\s*5[\s\S]*FillYoloStableFinalMaskGaps[\s\S]*FillShortStableGaps\([\s\S]*_maskProvider[\s\S]*MaxGapFrames\s*=\s*YoloFinalMaskStableGapMaxFrames[\s\S]*BlockedSceneCarryFrameIndices\s*=\s*sceneCarryBlockedFrameIndices[\s\S]*YoloFinalMaskGapFill[\s\S]*blockedByCut=\{gapFill\.BlockedCutGapFaces\}[\s\S]*blockedByCleanup=\{gapFill\.BlockedCleanupGapFrames\}[\s\S]*blockedBySceneCarry=\{gapFill\.BlockedSceneCarryGapFrames\}[\s\S]*suppressedWeakGeometryAnchors=\{gapFill\.SuppressedWeakGeometryAnchorChecks\}[\s\S]*suppressedRiskyGeometryAnchors=\{gapFill\.SuppressedRiskyGeometryAnchorChecks\}[\s\S]*unsupportedWeakAnchors=\{gapFill\.UnsupportedWeakAnchorChecks\}"
...
Assert-Match "workspace carries cleanup removals and scene-cut carry windows into post-scene final gap fill" $workspaceText "yoloCleanupPass\s*=\s*RemoveYoloWeakIsolatedFinalMasks\(FrameList\.VideoPath,\s*token\)[\s\S]*RemoveSceneCutCarryRemnants\([\s\S]*_maskProvider[\s\S]*yoloCutPairs[\s\S]*BuildSceneCutCarryBlockedFrames\([\s\S]*yoloCutPairs[\s\S]*YoloSceneCutCarryBlockFrames[\s\S]*additionalBlockedFrameIndices:\s*CombineFrameIndices\([\s\S]*yoloCleanupPass\.RemovedFrameIndices[\s\S]*yoloCarryCleanup\.RemovedFrameIndices[\s\S]*sceneCarryBlockedFrameIndices:\s*yoloSceneCutBlockedFrames[\s\S]*CombineFrameIndices\(additionalBlockedFrameIndices,\s*cleanup\.RemovedFrameIndices\)"
Assert-Match "workspace carries initial gap-fill cut pairs into post-scene final gap fill" $workspaceText "var\s+yoloCutPairs\s*=\s*CombineCutFramePairs\([\s\S]*yoloPreSmoothCutPairs[\s\S]*yoloPostSmoothCutPairs[\s\S]*yoloCleanupPass\.CutFramePairs[\s\S]*var\s+gapFillCutPairs[\s\S]*gapFillCutPairs\s*=\s*FillYoloStableFinalMaskGaps[\s\S]*return\s+new\s+YoloFinalMaskCleanupPassResult\(cleanup\.RemovedFrameIndices,\s*gapFillCutPairs\)[\s\S]*YoloFinalMaskCleanupPassResult[\s\S]*CutFramePairs"
...
Assert-Match "final mask postprocessor removes unsupported strong scene carry candidates" $finalMaskPostProcessorText "IsUnsupportedStrongCarryLikeFace[\s\S]*StrongCarrySupportLookaheadFrames[\s\S]*StrongCarrySupportMinConfidence[\s\S]*RemovedUnsupportedStrongCarryLikeFaces[\s\S]*RemovedUnsupportedStrongCarryLikeFrameIndices"
...
Assert-Match "final mask scene carry cleanup uses source lookback" $finalMaskPostProcessorText "GetSceneCutCarryReferences[\s\S]*cutStartFrame\s*-\s*lookbackFrames[\s\S]*SourceLookbackFrames\s*\{\s*get;\s*init;\s*\}\s*=\s*2"
Assert-Match "final mask cleanup verifies scene-cut carry cleanup behavior" $finalMaskCleanupVerifyText "RemoveSceneCutCarryRemnants[\s\S]*unsupported strong carry frame 1006[\s\S]*Expected scene-cut carry cleanup to remove unsupported strong carry-like frames 1006 and 2009[\s\S]*Expected scene-cut carry cleanup not to recreate removed carry masks[\s\S]*Expected high-confidence post-cut anchor 3002 with later support to be reported as protected carry-like review candidate"
...
Assert-Match "final mask gap fill blocks whole cleanup-contaminated gaps" $finalMaskPostProcessorText "blockedFrames\s*=\s*new\s+HashSet<int>\(options\.BlockedFrameIndices\)[\s\S]*GetUnstoredGapFrameIndices\(previousFrame,\s*nextFrame,\s*storedFrames\)[\s\S]*hasCleanupBlockedFrame\s*=\s*gapFrameIndices\.Any\(blockedFrames\.Contains\)[\s\S]*blockedCleanupFrames\.Add\(frameIndex\)[\s\S]*BlockedCleanupGapFrames[\s\S]*BlockedCleanupFrameIndices"
Assert-Match "final mask gap fill blocks whole scene-carry-contaminated gaps" $finalMaskPostProcessorText "blockedSceneCarryFrames\s*=\s*new\s+HashSet<int>\(options\.BlockedSceneCarryFrameIndices\)[\s\S]*GetUnstoredGapFrameIndices\(previousFrame,\s*nextFrame,\s*storedFrames\)[\s\S]*hasSceneCarryBlockedFrame\s*=\s*gapFrameIndices\.Any\(blockedSceneCarryFrames\.Contains\)[\s\S]*blockedSceneCarryGapFrames\.Add\(frameIndex\)[\s\S]*BlockedSceneCarryGapFrames[\s\S]*BlockedSceneCarryFrameIndices"
...
Assert-Match "final mask cleanup verifies whole-gap cleanup and scene-carry blocks" $finalMaskCleanupVerifyText "partialBlockedFrameGapFill[\s\S]*Expected a cleanup-blocked frame inside a stable gap to block the whole gap[\s\S]*partialPostCutWindowGapFill[\s\S]*Expected one scene-carry blocked frame inside a stable gap to block the whole gap"
...
Assert-Match "smoke harness matches yolo scene cut threshold" $smokeHarnessText "yoloSceneCutDirectCarryMaxConfidence\s*=\s*0\.95f[\s\S]*yoloSceneCutDirectCarryMinSourceConfidence\s*=\s*0\.58f[\s\S]*yoloSceneCutPostCutCarryMaxConfidence\s*=\s*0\.78f[\s\S]*yoloSceneCutDifferenceThreshold\s*=\s*0\.15[\s\S]*yoloSceneCutDirectDifferenceThreshold\s*=\s*0\.32[\s\S]*yoloSceneCutDirectDifferenceMaxCandidates\s*=\s*48[\s\S]*yoloSceneCutMatchingTailMaxFrames\s*=\s*5[\s\S]*yoloSceneCutMatchingTailMaxConfidence\s*=\s*0\.95f[\s\S]*yoloSceneCutCandidateMatchMinIou\s*=\s*0\.55[\s\S]*yoloSceneCutCandidateMatchMaxCenterShiftRatio\s*=\s*0\.65[\s\S]*yoloSceneCutCandidateMatchMaxAreaChangeRatio\s*=\s*3\.0[\s\S]*yoloSceneCutPostCutLookbackFrames\s*=\s*3[\s\S]*minConfidenceDrop:\s*0\.0f[\s\S]*maxPostCutCarryFrames:\s*5[\s\S]*minSourceConfidence:\s*yoloSceneCutDirectCarryMinSourceConfidence[\s\S]*maxCarryFrames:\s*5[\s\S]*sourceLookbackFrames:\s*yoloSceneCutPostCutLookbackFrames[\s\S]*differenceThreshold:\s*yoloSceneCutDifferenceThreshold[\s\S]*directDifferenceThreshold:\s*yoloSceneCutDirectDifferenceThreshold[\s\S]*directDifferenceMaxChecks:\s*yoloSceneCutDirectDifferenceMaxCandidates[\s\S]*removeMatchingTailFrames:\s*yoloSceneCutMatchingTailMaxFrames[\s\S]*removeMatchingTailMaxConfidence:\s*yoloSceneCutMatchingTailMaxConfidence[\s\S]*candidateMatchMinIou:\s*yoloSceneCutCandidateMatchMinIou"
...
Assert-Match "smoke harness logs yolo final gap fill" $smokeHarnessText "yoloFinalMaskStableGapMaxFrames\s*=\s*5[\s\S]*FillShortStableGaps\([\s\S]*maskProvider[\s\S]*MaxGapFrames\s*=\s*yoloFinalMaskStableGapMaxFrames[\s\S]*SmokeYoloFinalMaskGapFill[\s\S]*blockedByCut=\{gapFill\.BlockedCutGapFaces\}[\s\S]*blockedByCleanup=\{gapFill\.BlockedCleanupGapFrames\}[\s\S]*suppressedWeakGeometryAnchors=\{gapFill\.SuppressedWeakGeometryAnchorChecks\}[\s\S]*suppressedRiskyGeometryAnchors=\{gapFill\.SuppressedRiskyGeometryAnchorChecks\}[\s\S]*unsupportedWeakAnchors=\{gapFill\.UnsupportedWeakAnchorChecks\}"
Assert-Match "smoke harness blocks final gap fill from cleanup removals and scene-cut carry windows" $smokeHarnessText "cleanupBlockedFrameIndices\s*=\s*cleanup\.RemovedFrameIndices[\s\S]*BlockedFrameIndices\s*=\s*cleanup\.RemovedFrameIndices[\s\S]*sceneCutBlockedFrameIndices\s*=\s*YoloFinalMaskPostProcessor\.BuildSceneCutCarryBlockedFrames[\s\S]*postSceneBlockedFrameIndices\s*=\s*cleanupBlockedFrameIndices[\s\S]*Concat\(postSceneCleanup\.RemovedFrameIndices\)[\s\S]*BlockedFrameIndices\s*=\s*postSceneBlockedFrameIndices[\s\S]*BlockedSceneCarryFrameIndices\s*=\s*sceneCutBlockedFrameIndices"
