...
Assert-Match "final mask cleanup verifies scene-cut carry cleanup behavior" $finalMaskCleanupVerifyText "RemoveSceneCutCarryRemnants[\s\S]*Expected scene-cut carry cleanup to remove weak frames 1001-1005, extended weak frames 1007-1008, and direct-pair frames 2001-2008[\s\S]*Expected scene-cut carry cleanup frames to block final gap refill"
...
Assert-Match "smoke harness cleans weak remnants after scene guard" $smokeHarnessText "yoloSceneCutCarryBlockFrames\s*=\s*8[\s\S]*yoloSceneCutExtendedWeakCarryMaxConfidence\s*=\s*0\.78f[\s\S]*SmokeFaceTrackSceneCutGuard[\s\S]*RemoveSceneCutCarryRemnants[\s\S]*ExtendedWeakCarryFrames\s*=\s*yoloSceneCutCarryBlockFrames[\s\S]*ExtendedWeakMaxConfidence\s*=\s*yoloSceneCutExtendedWeakCarryMaxConfidence[\s\S]*postProcessor\.RemoveWeakIsolatedMasks\(maskProvider\)[\s\S]*sceneCutBlockedFrameIndices\s*=\s*YoloFinalMaskPostProcessor\.BuildSceneCutCarryBlockedFrames[\s\S]*yoloSceneCutCarryBlockFrames[\s\S]*SmokeYoloSceneCutCarryCleanup[\s\S]*blockedFrames=\{FormatFrames\(sceneCutBlockedFrameIndices\)\}[\s\S]*blockFrames=\{yoloSceneCutCarryBlockFrames\}[\s\S]*extendedWeakMaxConfidence=\{yoloSceneCutExtendedWeakCarryMaxConfidence[\s\S]*SmokeYoloFinalMaskPostSceneCleanup"
