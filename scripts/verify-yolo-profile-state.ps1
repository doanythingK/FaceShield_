...
Assert-Match "workspace carries cleanup removals into post-scene final gap fill" $workspaceText "yoloCleanupBlockedFrames\s*=\s*RemoveYoloWeakIsolatedFinalMasks\(FrameList\.VideoPath,\s*token\)[\s\S]*additionalBlockedFrameIndices:\s*yoloCleanupBlockedFrames[\s\S]*CombineFrameIndices\(additionalBlockedFrameIndices,\s*cleanup\.RemovedFrameIndices\)"
...
Assert-Match "smoke harness refills after scene guard with blocked cut pairs" $smokeHarnessText "SmokeYoloFinalMaskPostSceneCleanup[\s\S]*FillShortStableGaps\([\s\S]*BlockedCutFramePairs\s*=\s*sceneCut\.CutFramePairs[\s\S]*SmokeYoloFinalMaskPostSceneGapFill[\s\S]*blockedByCut=\{postSceneGapFill\.BlockedCutGapFaces\}[\s\S]*blockedByCleanup=\{postSceneGapFill\.BlockedCleanupGapFrames\}[\s\S]*SmokeYoloFinalMaskPostSceneGapFillSceneCutGuard"
