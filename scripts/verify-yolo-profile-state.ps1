...
Assert-Match "workspace carries initial gap-fill cut pairs into post-scene final gap fill" $workspaceText "var\s+yoloCutPairs\s*=\s*CombineCutFramePairs\([\s\S]*yoloPreSmoothCutPairs[\s\S]*yoloPostSmoothCutPairs[\s\S]*yoloCleanupPass\.CutFramePairs[\s\S]*var\s+gapFillCutPairs[\s\S]*gapFillCutPairs\s*=\s*FillYoloStableFinalMaskGaps[\s\S]*return\s+new\s+YoloFinalMaskCleanupPassResult\(cleanup\.RemovedFrameIndices,\s*cleanup\.RemovedFacesInfo,\s*gapFillCutPairs\)[\s\S]*YoloFinalMaskCleanupPassResult[\s\S]*RemovedFacesInfo[\s\S]*CutFramePairs"
...
Assert-Match "final mask gap fill blocks scene-carry-contaminated frames" $finalMaskPostProcessorText "blockedSceneCarryFrames\s*=\s*new\s+HashSet<int>\(options\.BlockedSceneCarryFrameIndices\)[\s\S]*blockedSceneCarryFacesByFrame\s*=\s*BuildBlockedFacesByFrame\(options\.BlockedSceneCarryFaces\)[\s\S]*blockedBySceneCarry\s*=\s*blockedSceneCarryFrames\.Contains\(frameIndex\)[\s\S]*blockedSceneCarryGapFrames\.Add\(frameIndex\)[\s\S]*BlockedSceneCarryGapFrames[\s\S]*BlockedSceneCarryFrameIndices"
...
Assert-Match "final mask cleanup verifies whole-gap cleanup and per-frame scene-carry blocks" $finalMaskCleanupVerifyText "partialBlockedFrameGapFill[\s\S]*Expected a cleanup-blocked frame inside a stable gap to block the whole gap[\s\S]*partialPostCutWindowGapFill[\s\S]*Expected one scene-carry blocked frame inside a stable gap to suppress only that frame"
