...
Assert-Match "smoke harness guards yolo final gap fill across scene cuts" $smokeHarnessText "FillShortStableGaps\(maskProvider\)[\s\S]*gapFill\.CutGuardFacesInfo[\s\S]*FaceTrackSceneCutGuard\(\)\.Apply[\s\S]*SmokeYoloFinalMaskGapFillSceneCutGuard"
