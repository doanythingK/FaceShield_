...
Assert-Match "workspace purges residual yolo carry masks after scene cuts" $workspaceText "YoloSceneCutCarryPurgeFrames\s*=\s*5[\s\S]*YoloSceneCutCarryPurgeMaxConfidence\s*=\s*0\.95f[\s\S]*RemoveSceneCutCarryRemnants\([\s\S]*new\s+YoloSceneCutCarryCleanupOptions[\s\S]*YoloSceneCutCarryCleanup[\s\S]*blockedFrames=\{FormatFrameList\(yoloSceneCutBlockedFrames\)\}"
...
Assert-Match "smoke harness cleans weak remnants after scene guard" $smokeHarnessText "SmokeFaceTrackSceneCutGuard[\s\S]*RemoveSceneCutCarryRemnants[\s\S]*postProcessor\.RemoveWeakIsolatedMasks\(maskProvider\)[\s\S]*sceneCutBlockedFrameIndices\s*=\s*YoloFinalMaskPostProcessor\.BuildSceneCutCarryBlockedFrames[\s\S]*SmokeYoloSceneCutCarryCleanup[\s\S]*blockedFrames=\{FormatFrames\(sceneCutBlockedFrameIndices\)\}[\s\S]*SmokeYoloFinalMaskPostSceneCleanup"
