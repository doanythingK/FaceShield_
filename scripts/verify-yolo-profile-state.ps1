...
Assert-Match "final mask gap fill reports scene-carry-blocked frames separately" $finalMaskPostProcessorText "blockedSceneCarryFrames\s*=\s*new\s+HashSet<int>\(options\.BlockedSceneCarryFrameIndices\)[\s\S]*blockedSceneCarryGapFrames[\s\S]*blockedSceneCarryFrames\.Contains\(frameIndex\)[\s\S]*blockedSceneCarryGapFrames\.Add\(frameIndex\)[\s\S]*BlockedSceneCarryGapFrames[\s\S]*BlockedSceneCarryFrameIndices"
...
Assert-Match "plan records medium risky anchor support rule" $autoMosaicQualityPlanText "medium-confidence geometry-risk anchors[\s\S]*third matching temporal support[\s\S]*mediumRiskyAnchorGapFilled=0[\s\S]*supportedMediumRiskyAnchorGapFilled=2"
