...
Assert-Match "final mask gap fill skips cleanup-blocked frames" $finalMaskPostProcessorText "blockedFrames\s*=\s*new\s+HashSet<int>\(options\.BlockedFrameIndices\)[\s\S]*blockedFrames\.Contains\(frameIndex\)[\s\S]*BlockedFrameIndices"
...
Assert-Match "smoke harness refills after scene guard with blocked cut pairs" $smokeHarnessText "SmokeYoloFinalMaskPostSceneCleanup[\s\S]*FillShortStableGaps\([\s\S]*BlockedCutFramePairs\s*=\s*sceneCut\.CutFramePairs[\s\S]*SmokeYoloFinalMaskPostSceneGapFill[\s\S]*blockedByCut=\{postSceneGapFill\.BlockedCutGapFaces\}[\s\S]*SmokeYoloFinalMaskPostSceneGapFillSceneCutGuard"
