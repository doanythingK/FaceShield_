...
Assert-Match "workspace smoothing does not search across long scene gaps" $workspaceText "TemporalSmoothSearchWindowFrames\s*=\s*2[\s\S]*FindNearestTemporalFaces\(facesByFrame,\s*i,\s*-1,\s*TemporalSmoothSearchWindowFrames\)[\s\S]*FindNearestTemporalFaces\(facesByFrame,\s*i,\s*1,\s*TemporalSmoothSearchWindowFrames\)[\s\S]*searched\s*>\s*maxDistanceFrames"
...
Assert-Match "workspace broadens yolo scene cut carry candidates conservatively" $workspaceText "YoloSceneCutDirectCarryMaxConfidence\s*=\s*0\.78f[\s\S]*YoloSceneCutPostCutCarryMaxConfidence\s*=\s*0\.78f[\s\S]*YoloSceneCutDifferenceThreshold\s*=\s*0\.15[\s\S]*YoloSceneCutDirectDifferenceThreshold\s*=\s*0\.15[\s\S]*YoloSceneCutMatchingTailMaxFrames\s*=\s*5[\s\S]*YoloSceneCutMatchingTailMaxConfidence\s*=\s*0\.78f[\s\S]*BuildWeakTrackTransitionCandidates[\s\S]*maxTargetConfidence:\s*YoloSceneCutDirectCarryMaxConfidence[\s\S]*minConfidenceDrop:\s*0\.06f[\s\S]*maxPostCutCarryFrames:\s*5[\s\S]*BuildWeakPostCutCarryCandidates[\s\S]*maxTargetConfidence:\s*YoloSceneCutPostCutCarryMaxConfidence[\s\S]*maxCarryFrames:\s*5[\s\S]*removeMatchingTailFrames:\s*YoloSceneCutMatchingTailMaxFrames[\s\S]*removeMatchingTailMaxConfidence:\s*YoloSceneCutMatchingTailMaxConfidence"
...
Assert-Match "workspace applies yolo scene cut after temporal smoothing" $workspaceText "var\s+trackPost\s*=\s*ApplyAutoTemporalFixes\(\);[\s\S]*RefineAutoFacesWithRoi[\s\S]*ApplyAutoTemporalSmoothing\(\);[\s\S]*RemoveYoloTrackFillAcrossSceneCuts\(FrameList\.VideoPath,\s*trackPost,\s*token\);[\s\S]*RemoveYoloWeakIsolatedFinalMasks\(FrameList\.VideoPath,\s*token,\s*fillStableGaps:\s*false\);[\s\S]*LogFinalMaskSummary"
