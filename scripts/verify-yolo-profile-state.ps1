...
Assert-Match "workspace purges residual yolo carry masks after scene cuts" $workspaceText "YoloSceneCutCarryPurgeFrames\s*=\s*5[\s\S]*YoloSceneCutCarryBlockFrames\s*=\s*8[\s\S]*YoloSceneCutCarryPurgeMaxConfidence\s*=\s*0\.95f[\s\S]*RemoveSceneCutCarryRemnants\([\s\S]*new\s+YoloSceneCutCarryCleanupOptions[\s\S]*BuildSceneCutCarryBlockedFrames\([\s\S]*YoloSceneCutCarryBlockFrames[\s\S]*YoloSceneCutCarryCleanup[\s\S]*blockedFrames=\{FormatFrameList\(yoloSceneCutBlockedFrames\)\}[\s\S]*blockFrames=\{YoloSceneCutCarryBlockFrames\}"
...
Assert-Match "smoke harness cleans weak remnants after scene guard" $smokeHarnessText "yoloSceneCutCarryBlockFrames\s*=\s*8[\s\S]*SmokeFaceTrackSceneCutGuard[\s\S]*RemoveSceneCutCarryRemnants[\s\S]*postProcessor\.RemoveWeakIsolatedMasks\(maskProvider\)[\s\S]*sceneCutBlockedFrameIndices\s*=\s*YoloFinalMaskPostProcessor\.BuildSceneCutCarryBlockedFrames[\s\S]*yoloSceneCutCarryBlockFrames[\s\S]*SmokeYoloSceneCutCarryCleanup[\s\S]*blockedFrames=\{FormatFrames\(sceneCutBlockedFrameIndices\)\}[\s\S]*blockFrames=\{yoloSceneCutCarryBlockFrames\}[\s\S]*SmokeYoloFinalMaskPostSceneCleanup"
