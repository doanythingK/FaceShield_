...
Assert-Match "workspace skips postprocess for incomplete auto runs" $workspaceText "AutoDetectionCompletionTailToleranceFrames\s*=\s*2[\s\S]*IsAutoDetectionRunComplete\(generator\.LastRunSummary,\s*lastProcessed,\s*FrameList\.TotalFrames\)[\s\S]*AutoMaskPostProcessSkipped[\s\S]*return\s+false;[\s\S]*AutoMaskRunSummary\?\s+summary[\s\S]*summaryLastFrame\s*=\s*summary\.StartFrameIndex\s*\+\s*summary\.ProcessedFrames\s*-\s*1"
...
Assert-Match "docs record yolo direct scene cut budget" $yoloGuiSmokeResultText "direct source->target frame difference[\s\S]*cap direct source->target checks at 160"
Assert-Match "plan records yolo direct scene cut budget" $autoMosaicQualityPlanText "YoloSceneCutDirectDifferenceMaxCandidates=160[\s\S]*cap expensive direct source-to-target frame comparisons at 160"
