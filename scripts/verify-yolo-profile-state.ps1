...
Assert-Match "workspace logs yolo final cleanup reasons" $workspaceText "removedWeakUnsupported=\{cleanup\.RemovedWeakUnsupportedFaces\}[\s\S]*removedWeakShortClusters=\{cleanup\.RemovedWeakShortClusterFaces\}[\s\S]*removedWeakTinyClusters=\{cleanup\.RemovedWeakTinyClusterFaces\}[\s\S]*removedTinyShortClusters=\{cleanup\.RemovedTinyShortClusterFaces\}[\s\S]*removedTinyIsolated=\{cleanup\.RemovedTinyIsolatedFaces\}[\s\S]*removedUpperWeakClusters=\{cleanup\.RemovedUpperWeakClusterFaces\}"
Assert-Match "workspace can run final cleanup without refilling after scene guard" $workspaceText "RemoveYoloWeakIsolatedFinalMasks\(FrameList\.VideoPath,\s*token,\s*fillStableGaps:\s*false\)[\s\S]*bool\s+fillStableGaps\s*=\s*true[\s\S]*if\s*\(fillStableGaps\)[\s\S]*FillYoloStableFinalMaskGaps"
Assert-Match "workspace fills stable yolo final gaps" $workspaceText "YoloFinalMaskStableGapMaxFrames\s*=\s*5[\s\S]*FillYoloStableFinalMaskGaps[\s\S]*FillShortStableGaps\([\s\S]*_maskProvider[\s\S]*MaxGapFrames\s*=\s*YoloFinalMaskStableGapMaxFrames[\s\S]*YoloFinalMaskGapFill"
...
Assert-Match "workspace broadens yolo scene cut carry candidates conservatively" $workspaceText "YoloSceneCutDirectCarryMaxConfidence\s*=\s*0\.90f[\s\S]*YoloSceneCutPostCutCarryMaxConfidence\s*=\s*0\.78f[\s\S]*YoloSceneCutDifferenceThreshold\s*=\s*0\.15[\s\S]*YoloSceneCutDirectDifferenceThreshold\s*=\s*0\.15[\s\S]*YoloSceneCutMatchingTailMaxFrames\s*=\s*5[\s\S]*YoloSceneCutMatchingTailMaxConfidence\s*=\s*0\.90f[\s\S]*YoloSceneCutCandidateMatchMinIou\s*=\s*0\.55[\s\S]*YoloSceneCutCandidateMatchMaxCenterShiftRatio\s*=\s*0\.45[\s\S]*YoloSceneCutCandidateMatchMaxAreaChangeRatio\s*=\s*2\.0[\s\S]*YoloSceneCutPostCutLookbackFrames\s*=\s*3[\s\S]*BuildWeakTrackTransitionCandidates[\s\S]*maxTargetConfidence:\s*YoloSceneCutDirectCarryMaxConfidence[\s\S]*minConfidenceDrop:\s*0\.06f[\s\S]*maxPostCutCarryFrames:\s*5[\s\S]*BuildWeakPostCutCarryCandidates[\s\S]*maxTargetConfidence:\s*YoloSceneCutPostCutCarryMaxConfidence[\s\S]*maxCarryFrames:\s*5[\s\S]*sourceLookbackFrames:\s*YoloSceneCutPostCutLookbackFrames[\s\S]*includeEdgeCandidates:\s*true[\s\S]*removeMatchingTailFrames:\s*YoloSceneCutMatchingTailMaxFrames[\s\S]*removeMatchingTailMaxConfidence:\s*YoloSceneCutMatchingTailMaxConfidence[\s\S]*candidateMatchMinIou:\s*YoloSceneCutCandidateMatchMinIou"
...
Assert-Match "scene cut guard builds weak post-cut carry candidates" $sceneCutGuardText "BuildWeakPostCutCarryCandidates[\s\S]*maxCarryFrames[\s\S]*sourceLookbackFrames[\s\S]*includeEdgeCandidates[\s\S]*BuildWeakCarryRun[\s\S]*HasStrongContinuation[\s\S]*lookbackFrames[\s\S]*sourceFrame[\s\S]*FaceTrackFilledFace"
...
Assert-Match "smoke harness matches yolo scene cut threshold" $smokeHarnessText "yoloSceneCutPostCutCarryMaxConfidence\s*=\s*0\.78f[\s\S]*yoloSceneCutDifferenceThreshold\s*=\s*0\.15[\s\S]*yoloSceneCutDirectDifferenceThreshold\s*=\s*0\.15[\s\S]*yoloSceneCutMatchingTailMaxFrames\s*=\s*5[\s\S]*yoloSceneCutMatchingTailMaxConfidence\s*=\s*0\.90f[\s\S]*yoloSceneCutCandidateMatchMinIou\s*=\s*0\.55[\s\S]*yoloSceneCutCandidateMatchMaxCenterShiftRatio\s*=\s*0\.45[\s\S]*yoloSceneCutCandidateMatchMaxAreaChangeRatio\s*=\s*2\.0[\s\S]*yoloSceneCutPostCutLookbackFrames\s*=\s*3[\s\S]*sourceLookbackFrames:\s*yoloSceneCutPostCutLookbackFrames[\s\S]*differenceThreshold:\s*yoloSceneCutDifferenceThreshold[\s\S]*directDifferenceThreshold:\s*yoloSceneCutDirectDifferenceThreshold[\s\S]*removeMatchingTailFrames:\s*yoloSceneCutMatchingTailMaxFrames[\s\S]*removeMatchingTailMaxConfidence:\s*yoloSceneCutMatchingTailMaxConfidence[\s\S]*candidateMatchMinIou:\s*yoloSceneCutCandidateMatchMinIou"
...
Assert-Match "smoke harness logs yolo final gap fill" $smokeHarnessText "yoloFinalMaskStableGapMaxFrames\s*=\s*5[\s\S]*FillShortStableGaps\([\s\S]*maskProvider[\s\S]*MaxGapFrames\s*=\s*yoloFinalMaskStableGapMaxFrames[\s\S]*SmokeYoloFinalMaskGapFill"
