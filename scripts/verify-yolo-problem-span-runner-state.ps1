...
Assert-Match "smoke result records current high-confidence validation" $smoke 'Current validation after the high-confidence scene-carry, medium-risk anchor, whole-gap block, inward-initial-fill, 8-frame scene-carry refill-block, and extended weak carry cleanup update[\s\S]*verify-face-track-postprocess\.ps1[\s\S]*outwardInitialFilled=False[\s\S]*blockedInitialFill=1[\s\S]*verify-yolo-final-mask-cleanup\.ps1[\s\S]*mediumRiskyAnchorGapFilled=0[\s\S]*mediumRiskyAnchorSuppressed=1[\s\S]*supportedMediumRiskyAnchorGapFilled=2[\s\S]*partialCleanupBlockedGapFilled=0[\s\S]*partialSceneCarryRefillBlocked=3[\s\S]*sceneCutCarryRemoved=15[\s\S]*sceneCutCarryBlockedFrames=1001,1002,1003,1004,1005,1006,1007,1008,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011[\s\S]*dotnet build FaceShield\.sln[\s\S]*SmokeQualityGate passed=True[\s\S]*avgBestIou=1\.000[\s\S]*largeJumpGaps=0'
