...
Assert-Match "runner forwards pseudo gt output csv" $runner '\[string\]\$PseudoGtOutputCsv[\s\S]*-PseudoGtOutputCsv'
Assert-Match "runner forwards pseudo gt summary path" $runner '\[string\]\$PseudoGtSummaryPath[\s\S]*-PseudoGtSummaryPath'
Assert-Match "runner forwards pseudo gt review queue csv" $runner '\[string\]\$PseudoGtReviewQueueCsv[\s\S]*-PseudoGtReviewQueueCsv'
Assert-Match "runner publishes pseudo gt to goal evidence paths" $runner '\[switch\]\$PublishPseudoGtToGoalEvidence[\s\S]*\.tmp\\yolo-pseudo-gt\\pseudo-gt-candidates\.csv[\s\S]*\.tmp\\yolo-pseudo-gt\\pseudo-gt-review-queue\.csv'
Assert-Match "runner refuses publish without face evidence" $runner 'PublishPseudoGtToGoalEvidence requires tile-face or face-verification evidence'
...
Assert-Match "runner defaults pseudo gt tile external timeout" $runner '\[int\]\$PseudoGtTileExternalTimeoutSeconds\s*=\s*180'
...
Assert-Match "runner defaults pseudo gt face verification external timeout" $runner '\[int\]\$PseudoGtFaceVerificationExternalTimeoutSeconds\s*=\s*180'
...
Assert-Match "runner defaults pseudo gt person object external timeout" $runner '\[int\]\$PseudoGtPersonObjectExternalTimeoutSeconds\s*=\s*180'
...
Assert-Match "guide documents publishing pseudo gt to goal evidence" $guide 'PublishPseudoGtToGoalEvidence[\s\S]*\.tmp/yolo-pseudo-gt/pseudo-gt-candidates\.csv'
