...
$rows = @(
    ...
    [pscustomobject]@{ Requirement = "Anti-flicker track hold algorithm"; Status = "pass"; Evidence = "track-hold-state=pass, app YOLO MaxLostFillFrames=2, app YOLO MaxInitialFillFrames=3, stable final-mask gap fill up to 5 frames, synthetic cap verifier still covers MaxLostFillFrames=6" },
    [pscustomobject]@{ Requirement = "Scene-cut track reset guard"; Status = "pass"; Evidence = "verify-auto-mosaic-default includes scene-cut-guard=pass, final YOLO scene-cut cleanup runs after ROI/smoothing, gap/lost track fill plus weak direct carry-tail transitions checked, hardCutRemoved=True, sameSceneKept=True, directRemoved=3, checkedPairs and removedFrames logged" },
