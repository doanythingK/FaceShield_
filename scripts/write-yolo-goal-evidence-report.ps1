...
$rows = @(
    ...
    [pscustomobject]@{ Requirement = "Frame-level false-positive review evidence"; Status = "pass"; Evidence = "run-srcTest-smoke -DumpDetections emits parseable SmokeDetection lines with cx/cy/areaRatio/aspectRatio plus SmokeDetectionSummary; FinalMaskSummary/SmokeFinalMaskSummary now separates weakNonEdge, edgeWeak, and topEdgeWeak residuals; write-yolo-mask-continuity-report lists weak edge/top-edge review targets; new-yolo-full-gt-template/new-yolo-full-gt-review-package create crop and full-frame review artifacts for face/nonface/miss labeling; write-yolo-followup-quality-evidence wraps the follow-up evidence bundle" },
