...
$rows = @(
    ...
    [pscustomobject]@{ Requirement = "Anti-flicker track hold algorithm"; Status = "pass"; Evidence = "track-hold-state=pass, app YOLO MaxLostFillFrames=3, app YOLO MaxInitialFillFrames=3, YOLO synthetic track-fill confidence capped at 0.78 for scene-cut cleanup, sceneCutLostRemoved=6 verifies capped high-confidence lost-fill tails are cleared after a hard cut, stable final-mask gap fill up to 5 frames, synthetic cap verifier still covers MaxLostFillFrames=6" },
    [pscustomobject]@{ Requirement = "Scene-cut track reset guard"; Status = "pass"; Evidence = "verify-auto-mosaic-default includes scene-cut-guard=pass, final YOLO scene-cut cleanup runs after ROI/smoothing, post-scene weak cleanup runs without gap-fill, gap/lost track fill plus weak direct carry-tail transitions checked, hardCutRemoved=True, sameSceneKept=True, directRemoved=3, checkedPairs and removedFrames logged" },
    ...
    [pscustomobject]@{ Requirement = "Sparse/edge-tail temporal false-positive filter"; Status = "pass"; Evidence = "verify-auto-mosaic-default includes face-track-postprocess=pass, removedSparse=3, removedEdgeTail=1, YOLO sparse weak track profile active, low-confidence edge lost-fill blocked" },
    ...
    [pscustomobject]@{ Requirement = "Frame-level false-positive review evidence"; Status = "pass"; Evidence = "run-srcTest-smoke -DumpDetections emits parseable SmokeDetection lines with cx/cy/areaRatio/aspectRatio plus SmokeDetectionSummary; write-yolo-quality-review-checklist creates markdown review checklist; new-yolo-full-gt-template/new-yolo-full-gt-review-package create crop and full-frame review artifacts for face/nonface/miss labeling; write-yolo-followup-quality-evidence wraps the follow-up evidence bundle" },
