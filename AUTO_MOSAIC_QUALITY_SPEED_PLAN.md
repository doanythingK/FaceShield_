...
- `scripts/verify-face-track-scene-cut-guard.ps1` passed directly after the stronger source-match post-cut carry change: `directRemoved=3`, `mediumDirectRemoved=3`, `postCutCandidates=4`, `postCutRemoved=4`, `delayedPostCutCandidates=2`, `noSourcePostCutCandidates=0`, `gradualRemoved=1`, and `mildGradualRemoved=1`.
...
- A later 09:00 2-second fastcheck after adding stronger source-match post-cut carry filtering is `.tmp/yolo-followup-current-strong-source-0900-fastcheck/yolo-followup-quality-evidence.md`. It stayed on `detector=YoloFaceOnnxDetector/GPU:DirectML`, produced `99` final detection rows, kept frame range `4-59`, and reports `shortGaps=0`, `largeJumpGaps=0`, `isolated=0`, scene-cut guard `directCandidates=45`, `postCutCandidates=0`, `checked=57`, `maxDiff=0.097`, `cutPairs=none`, `removedFrames=none`, `elapsedMs=3391`, and low-confidence review frames `4,6,7,17,20,25,32`. This replaces the immediately previous broader-carry attempt that incorrectly removed frames `4,5,6,7` on the same no-hard-cut short sample.
