...
- Medium-confidence geometry-risk anchors (`tiny`, upper/lower weak region, or aspect outlier, `RiskyGeometryAnchorSupportMaxConfidence=0.72`) now require a third matching temporal support frame before they can seed YOLO final-mask gap fill. `scripts/verify-yolo-final-mask-cleanup.ps1` covers both sides as `mediumRiskyAnchorGapFilled=0` for two isolated risky hits and `supportedMediumRiskyAnchorGapFilled=2` with `supportedMediumRiskyAnchorGapFrames=921,923` when a third matching frame supports the run.
