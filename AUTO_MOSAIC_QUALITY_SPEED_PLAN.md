...
- The scene-cut guard now also builds weak post-cut carry candidates when a short non-edge low-confidence run appears immediately after a frame boundary without a compatible previous-frame face. These candidates are still removed only when the frame-difference check confirms a cut, and persistent weak runs beyond the carry cap are kept for review instead of being treated as automatic false positives.
...
- `scripts/verify-face-track-scene-cut-guard.ps1` covers hard-cut removal, same-scene preservation, weak direct transition removal, weak post-cut carry-run removal, and frame-diff cache behavior.
- The same scene-cut verifier now covers weak direct carry-tail removal (`directCandidates=3`, `directRemoved=3`) and weak post-cut carry-run removal (`postCutCandidates=2`, `postCutRemoved=2`) so only the first post-cut frame is not treated as the whole ghost-removal proof.
