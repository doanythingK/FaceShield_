...
- Residual scene-cut carry cleanup now looks back 3 frames for a pre-cut reference if the cut-start frame has no mask after earlier cleanup. This closes the case where a transition residue survives because the exact `source` frame in a confirmed cut pair was already empty.
