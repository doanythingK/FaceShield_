...
- `scripts/write-yolo-mask-continuity-report.ps1` now writes a final-mask continuity report from `[SmokeDetection]` rows. In this smoke harness those rows are the final `FrameMaskProvider` rectangles after tracking, scene-cut guard, and ROI refinement, so the report lists short empty gaps, isolated final-mask frames, low-confidence final masks, weak non-edge masks, weak edge masks, and top-edge weak masks as review targets for flicker/ghost/false-positive checks.
...
- The final-mask summary now also reports `weakNonEdge`, `edgeWeak`, `topEdgeWeak`, `tinyWeak`, and `tinyShort` row counts plus their frame lists after cleanup. These are residual review targets, not automatic errors: `weakNonEdge`/`tinyWeak`/`tinyShort` point to non-edge candidates that may be false positives, while `edgeWeak`/`topEdgeWeak` are separated because prior review showed top-edge partial faces can look like low-confidence false positives if removed blindly.
