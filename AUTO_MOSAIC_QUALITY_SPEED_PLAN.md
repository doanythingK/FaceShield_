...
- 2026-05-28 pseudo-GT closure evidence update: `close-yolo-pseudo-gt-review.ps1` now preserves person/object auxiliary evidence, repeated support, and IoU/center-distance/area-ratio fields in the closure CSV, so reviewed `face`/`nonface`/`miss` labels keep the evidence that produced each candidate.
