...
- Scene-cut carry cleanup now also reports high-confidence carry-like matches that were protected from automatic deletion as `protectedStrong` and `protectedStrongFrames`. This keeps real new-scene faces protected by default while making the remaining "mosaic survived the cut" review frames explicit in GUI and smoke logs.
- Follow-up evidence generation now adds `protectedStrongFrames` to required full-frame review frames. A cut-span review package therefore includes protected carry-like frames automatically, which lets visual review decide whether the protected box is a real new-scene face or a remaining scene-transition false positive.
...
- 2026-05-27 protected-carry fastcheck used the same 2-second clip and wrote `.tmp/yolo-followup-current-protected-carry-fastcheck/yolo-followup-quality-evidence.md`. The scene-cut carry cleanup line now includes `protectedStrong=0` and `protectedStrongFrames=none` on this no-cut sample. On a real cut span, nonzero protected frames mean the cleanup found carry-like boxes after a confirmed cut but kept them because their confidence exceeded the automatic deletion threshold; those frames are now also added to the full-frame review set.
