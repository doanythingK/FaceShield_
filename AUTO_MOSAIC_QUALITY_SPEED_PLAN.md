...
- `scripts/verify-yolo-final-mask-cleanup.ps1` now also verifies scene-carry frame blocking with `partialSceneCarryRefillBlocked=3` and `partialSceneCarryBlockedFrames=3101,3102,3103`, proving a blocked transition-carry frame suppresses the whole stable gap instead of allowing adjacent frames to be recreated. It also verifies scene-carry anchor blocking with `sceneCarryAnchorRefillBlocked=1` and `sceneCarryAnchorBlockedFrames=3209`, proving a protected cut-window anchor cannot seed a new fill just outside the block window. In the app, the full scene-carry window from confirmed cuts is still passed into `BlockedSceneCarryFrameIndices`, so immediate cut windows remain protected from anti-flicker recreation.
- 2026-05-27 whole-gap scene-carry block validation: `scripts/verify-yolo-final-mask-cleanup.ps1` passed with `partialSceneCarryRefillBlocked=3`, `partialSceneCarryBlockedFrames=3101,3102,3103`, `sceneCarryAnchorRefillBlocked=1`, and `sceneCarryAnchorBlockedFrames=3209`; `scripts/verify-yolo-profile-state.ps1` passed; Windows `dotnet build C:\testdev\FaceShield.sln` passed with 0 errors and existing FFmpeg obsolete-field warnings; and `scripts/verify-auto-mosaic-default.ps1` passed with the FaceONNX/default gate still at `SmokeQualityGate passed=True`, `baselineFrames=19`, `optimizedFrames=19`, `avgBestIou=1.000`, and `minBestIou=1.000`.
