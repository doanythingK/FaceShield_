...
- Frames removed by YOLO final-mask cleanup are now reported separately when a later stable gap-fill pass skips them: `blockedByCleanup=...` and `cleanupBlockedFrames=...`. This gives direct run-log evidence that anti-flicker filling did not recreate a weak false-positive cleanup removal, and `frames=...` now reports only newly filled frames rather than candidate/existing frames.
...
- `scripts/verify-yolo-final-mask-cleanup.ps1` now also verifies cleanup-blocked gap fill with `cleanupBlockedGapFilled=0`, `cleanupBlockedGapFrames=1`, and `cleanupBlockedFrames=701`, proving a frame removed by final-mask cleanup cannot be recreated by the stable anti-flicker fill pass and is reported in logs.
