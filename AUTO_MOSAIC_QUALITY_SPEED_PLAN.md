...
- `scripts/write-yolo-mask-continuity-report.ps1` now writes a final-mask continuity report from `[SmokeDetection]` rows. In this smoke harness those rows are the final `FrameMaskProvider` rectangles after tracking, scene-cut guard, and ROI refinement, so the report lists short empty gaps, isolated final-mask frames, low-confidence final masks, weak non-edge masks, weak edge masks, top-edge weak masks, upper weak non-edge masks, lower weak non-edge masks, and aspect-ratio outliers as review targets for flicker/ghost/false-positive checks.
