...
- 2026-05-26 update: the final GUI gap-fill pass now passes both pre-smooth and post-smooth YOLO scene-cut pairs into `FillShortStableGaps`. If a known cut pair sits between the two anchors, the gap is not filled at all; this prevents the last anti-flicker pass from recreating a blur that the scene-cut guard already removed. The GUI/smoke gap-fill logs now include `blockedByCut=...` and `blockedFrames=...`, so debug output can distinguish "missed flicker repair" from "intentionally skipped because the fill crossed a confirmed cut". `scripts/verify-yolo-final-mask-cleanup.ps1` covers this as `blockedCutGapFilled=0`, `blockedCutGapFaces=1`, and `blockedCutGapFrames=101`.
...
- `scripts/verify-yolo-final-mask-cleanup.ps1` now also verifies cleanup-blocked gap fill with `cleanupBlockedGapFilled=0`, proving a frame removed by final-mask cleanup cannot be recreated by the stable anti-flicker fill pass.
...
- The latest cleanup-block focused 2-second fastcheck is `.tmp/yolo-followup-current-combined-block-fastcheck/yolo-followup-quality-evidence.md`. It stayed on `detector=YoloFaceOnnxDetector/GPU:DirectML`, produced `96` final detection rows, and records `[SmokeYoloFinalMaskCleanup] ... removedUpperWeakClusters=3 ... removedFrames=33,34,35`, followed by `[SmokeYoloFinalMaskGapFill] ... filled=0` and `[SmokeYoloFinalMaskPostSceneGapFill] ... filled=0`. The final summary reports `shortGaps=0`, `largeJumpGaps=0`, `isolated=0`, `weakNonEdge=0`, and `upperWeak=0`, proving the earlier cleanup removals are not recreated by the later anti-flicker fill on this short sample.
