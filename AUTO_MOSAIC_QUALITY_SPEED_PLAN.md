...
- The same scene-cut verifier now covers weak direct carry-tail removal (`directCandidates=3`, `directRemoved=3`) and weak post-cut carry-run removal (`postCutCandidates=11`, `postCutRemoved=4`) so only the first post-cut frame is not treated as the whole ghost-removal proof.
...
- The final-mask summary now also reports `weakNonEdge` and `tinyWeak` row counts after cleanup. These are residual review targets, not automatic errors: they show weak non-edge boxes and tiny weak boxes that survived because they touched an edge, connected to stronger evidence, or otherwise failed the conservative removal rules.
- The GUI automatic mosaic path now also logs `[YoloFinalMaskCleanup] removedWeakIsolated=... removedWeakUnsupported=... removedWeakShortClusters=... removedWeakTinyClusters=... removedTinyIsolated=...` when the final cleanup removes weak or very small unsupported non-edge boxes. The smoke harness emits `[SmokeYoloFinalMaskCleanup] ...` with the same reason counts, and the follow-up evidence summary preserves that line.
- Final cleanup now also removes a very small, non-edge, medium-confidence candidate only when it has no matching temporal neighbor (`TinyIsolatedMaxConfidence=0.62`, `TinyIsolatedMaxAreaRatio=0.0009`). This targets one-frame tiny non-face boxes without removing high-confidence tiny candidates or temporally supported tiny runs.
- After YOLO final cleanup, the GUI path now fills only short final-mask gaps whose two anchors are both strong and geometrically stable. This targets residual flicker after tracking/cleanup without filling weak-anchor gaps or large-jump gaps; GUI logs `[YoloFinalMaskGapFill]`, and the smoke harness logs `[SmokeYoloFinalMaskGapFill]`.
- Each final-gap fill also carries source-frame evidence into the scene-cut guard, so a stable-looking gap fill is removed again if the frame-difference scan finds a cut between the source and filled frame. GUI logs this as `[YoloFinalMaskGapFillSceneCutGuard]`, the smoke harness logs `[SmokeYoloFinalMaskGapFillSceneCutGuard]`, and the focused verifier records the deterministic branch as `gapCutRemoved=1`.
- The YOLO scene-cut carry candidate range was widened from only weak detections to moderate-confidence carry candidates (`direct<=0.72`, post-cut short carry `<=0.58`). The guard still removes only when a frame-difference scan crosses the cut threshold, so this targets transition ghost masks without changing FaceONNX.
- The smoke harness now emits the equivalent `[SmokeFinalMaskSummary]` for YOLO runs, including `largeJumpGaps`/`largeJumpRanges`, and `scripts/write-yolo-followup-quality-evidence.ps1` records either `[SmokeFinalMaskSummary]` or GUI `[FinalMaskSummary]` in the generated evidence summary. This keeps GUI logs and CLI evidence aligned.
