...
- The final-mask summary now also reports `weakNonEdge`, `tinyWeak`, and `tinyShort` row counts after cleanup. These are residual review targets, not automatic errors: they show weak non-edge boxes, tiny weak boxes, and tiny weak-to-medium boxes that survived because they touched an edge, connected to stronger evidence, or otherwise failed the conservative removal rules.
...
- The smoke harness now emits the equivalent `[SmokeFinalMaskSummary]` for YOLO runs, including `largeJumpGaps`/`largeJumpRanges` and residual `tinyShort` review counts, and `scripts/write-yolo-followup-quality-evidence.ps1` records either `[SmokeFinalMaskSummary]` or GUI `[FinalMaskSummary]` in the generated evidence summary. This keeps GUI logs and CLI evidence aligned.
