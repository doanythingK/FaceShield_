...
- A YOLO-only final-mask cleanup now removes weak isolated non-edge final masks after ROI refinement. The cleanup is per-face, not just per-frame: a weak box now needs a matching neighboring box by IoU/center/area continuity to survive, so an unrelated weak box can be removed even when another real face track exists in adjacent frames. It still keeps matching adjacent weak masks for temporal review, strong isolated masks, and edge partial-face candidates so the cleanup does not repeat the risky top/small false-negative behavior.
...
- `scripts/verify-yolo-final-mask-cleanup.ps1` passed directly: weak isolated non-edge masks are removed, weak edge partial-face masks remain, strong isolated masks remain, adjacent matching weak masks remain, mixed frames keep the strong face, and a weak unrelated box is removed even when another face has temporal neighbors in adjacent frames.
