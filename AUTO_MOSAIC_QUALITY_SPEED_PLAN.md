...
- 2026-05-27 strong carry independence guard: high-confidence post-cut carry-like boxes now need later strong support that has clearly moved or changed scale away from the pre-cut reference before being protected. The synthetic verifier reports `stickyStrongCarryRemoved=5`, `stickyStrongCarryRemovedUnsupportedStrong=5`, and `driftingStrongCarryProtected=2`, so same-position or small-jitter high-confidence blur residue is removed after a confirmed cut while a clearly drifting supported post-cut face can still remain for review.
...
- 2026-05-27 scene-carry block extension: the direct high-confidence carry purge window remains 5 frames and covers same-geometry masks up to `0.98`, while matching weak carry masks at or below `0.78` are now removed through the 8-frame weak/block window. High-confidence carry-like boxes now need at least two later strong matching support frames before they are protected as possible real new-scene faces; otherwise they are removed as unsupported strong carry-like residue. The final anti-flicker gap-fill block window is also 8 frames. `scripts/verify-yolo-final-mask-cleanup.ps1` covers this with `sceneCutCarryFrames=...`, `emptyPostCutRemovedUnsupportedStrong=2`, `emptyPostCutProtected=0`, and extended `sceneCutCarryBlockedFrames=...`, so a confirmed transition can remove same-position residue and block later gap-fill recreation beyond the shorter high-confidence purge window without relying on confidence alone as proof.
