...
- `scripts/verify-yolo-manual-readiness-state.ps1`를 추가했다. 이 스크립트는 남은 수동 gate를 완료로 간주하지 않고, full-GT review package의 빈 라벨 CSV/crop/frame/overlay artifact, AI-assisted candidate CSV, GUI manual checklist, 10분 YOLO output/log, 미완료 FaceONNX baseline full attempt log가 다음 수동 검증을 진행할 수 있는 상태인지 확인한다.
...
<!-- yolo-manual-readiness-state: verifier=scripts/verify-yolo-manual-readiness-state.ps1; full-gt-review-package=ready-pending-human-labels; ai-reviewed-candidate=ready-not-final-gt; gui-checklist=ready-pending-human-smoke; ten-minute-artifacts=ready-yolo-output-and-incomplete-faceonnx-baseline -->
