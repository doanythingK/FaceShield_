...
- `scripts/verify-yolo-profile-state.ps1` now asserts that the smoke harness runs post-scene final gap fill with the scene-cut pairs, and `scripts/verify-yolo-followup-quality-evidence.ps1` asserts that the evidence parser accepts `[SmokeYoloFinalMaskPostSceneGapFill]` and `[SmokeYoloFinalMaskPostSceneGapFillSceneCutGuard]`.
...
<!-- yolo-followup-quality-state: default=FaceONNX-preserved; followup-scope=YOLO-only; anti-flicker=track-postprocess-hold-and-initial-fill-guarded; scene-cut=filled-initial-and-weak-direct-gapfill-blocked; sparse-scene-cut=guarded; false-positive=sparse-edge-tail-aspect-low-confidence-review-guarded; short-sample-review-package=ready; followup-evidence-wrapper=pass; problem-video-visual-confirmation=pending; complete=false; remaining=problem-video-visual-confirmation -->
