...
- The same scene-cut guard now also compares the carried source frame directly against the target frame (`source->target>=0.42`) after the adjacent-pair scan. This targets fade/transition ghost masks where no single adjacent frame pair reaches the hard-cut threshold but the held mask has moved into a different scene. `scripts/verify-face-track-scene-cut-guard.ps1` covers this as `gradualRemoved=1`.
