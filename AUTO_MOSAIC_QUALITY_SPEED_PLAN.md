...
- A YOLO-only scene-cut guard checks both filled gap/lost candidates and weak direct track transitions. Runtime logs include `directCandidates`, `checkedPairs`, `removedFrames`, and the frame-difference threshold.
...
- A YOLO-only final-mask cleanup now removes weak isolated non-edge final masks after ROI refinement. It keeps adjacent weak masks for temporal review, keeps strong isolated masks, and keeps edge partial-face candidates so the cleanup does not repeat the risky top/small false-negative behavior.
...
- `scripts/verify-face-track-scene-cut-guard.ps1` covers hard-cut removal, same-scene preservation, weak direct transition removal, and frame-diff cache behavior.
- The same scene-cut verifier now covers weak direct carry-tail removal (`directCandidates=3`, `directRemoved=3`) so only the first post-cut frame is not treated as the whole ghost-removal proof.
...
- The GUI automatic mosaic path now also logs `[YoloFinalMaskCleanup] removedWeakIsolated=...` when the final cleanup removes weak isolated non-edge boxes. The smoke harness emits `[SmokeYoloFinalMaskCleanup] ...` and the follow-up evidence summary preserves that line.
...
- `scripts/verify-yolo-followup-quality-evidence.ps1` passed and now asserts that generated evidence preserves both the final-mask cleanup line and the large-jump final-mask summary.
- `scripts/verify-yolo-final-mask-cleanup.ps1` passed directly: weak isolated non-edge masks are removed, weak edge partial-face masks remain, strong isolated masks remain, adjacent weak masks remain, and mixed frames keep the strong face.
...
- The latest 09:00 default evidence was regenerated after initial-fill wiring. The smoke summary preserved `[SmokeFaceTrackPost] ... initialFilled=3`, `[SmokeYoloFinalMaskCleanup] removedWeakIsolated=0`, `[SmokeFinalMaskSummary] ... shortGaps=0 ... largeJumpGaps=0 ... isolated=0 ... lowConf=7`, and review artifacts under `.tmp/yolo-followup-current-0900-default/review-package/` (`103` crop rows and `24` full-frame rows). The continuous overlay artifact is `.tmp/yolo-followup-current-0900-default/yolo-detection-overlay.mp4` with 103 overlays.
...
- Next completion evidence should include a run log from the problem clip plus visual review of the generated crop/frame package or equivalent screenshots/recording around the reported frames.

<!-- yolo-followup-quality-state: default=FaceONNX-preserved; followup-scope=YOLO-only; anti-flicker=track-postprocess-hold-and-initial-fill-guarded; scene-cut=filled-initial-and-weak-direct-guarded; sparse-scene-cut=guarded; false-positive=sparse-edge-tail-aspect-review-guarded; short-sample-review-package=ready; followup-evidence-wrapper=pass; problem-video-visual-confirmation=pending; complete=false; remaining=problem-video-visual-confirmation -->
