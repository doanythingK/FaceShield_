...
- YOLO scene-cut checks in the app and smoke harness now use `0.15` for adjacent-pair checks and `0.32` for direct source-to-target checks. This keeps the evidence path aligned with the GUI path for transition carry cleanup, and specifically catches the short-sample `maxDiff=0.158` carry through the adjacent-pair scan that the earlier `0.24` threshold left behind.
