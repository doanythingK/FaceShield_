...
- Deletion is still gated by the frame-difference guard, candidate geometry matching, and the `direct<=0.95` cap; this is intended to remove scene-transition residue without treating confidence alone as proof.
- Residual scene-cut carry cleanup now looks back 2 frames for a pre-cut reference if the cut-start frame has no mask after earlier cleanup. This closes the case where a transition residue survives because the exact `source` frame in a confirmed cut pair was already empty.
...
- 2026-05-27 scene-carry block extension: the direct high-confidence carry purge window remains 5 frames, while matching weak carry masks at or below `0.78` are now removed through the 8-frame weak/block window. The final anti-flicker gap-fill block window is also 8 frames. `scripts/verify-yolo-final-mask-cleanup.ps1` passed with `sceneCutCarryFrames=1001,1002,1003,1004,1005,1007,1008,2001,2002,2003,2004,2005,2006,2007,2008` and extended `sceneCutCarryBlockedFrames=1001,1002,1003,1004,1005,1006,1007,1008,2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2011`, so a confirmed transition can remove weak same-position residue and block later gap-fill recreation beyond the shorter high-confidence purge window without deleting stronger direct YOLO detections outside that purge window.
...
- 2026-05-26 high-confidence scene-carry update: YOLO confirmed-cut cleanup now treats same-geometry transition residue up to `0.95` confidence as removable in the direct transition candidate path, matching-tail cleanup, and final scene-carry purge. This still requires frame-difference cut evidence and geometry matching, so confidence alone is not treated as proof. The synthetic verifier now includes `0.93/0.94` carry frames to keep this regression covered.
