...
- Each GUI final-gap fill also carries source-frame evidence into `[YoloFinalMaskGapFillSceneCutGuard]`, so a stable-looking gap fill is removed again if the frame-difference scan finds a cut between the source and filled frame. The focused verifier records this as `gapCutRemoved=1`.
