...
- 2026-05-26 update: the GUI and smoke YOLO scene-cut path no longer disables this direct source->target comparison just because the candidate list is dense. Instead, it keeps the lower cumulative threshold enabled and caps direct checks at `YoloSceneCutDirectDifferenceMaxCandidates=24`; logs now include `directChecked=...` and `directSkipped=...`. This preserves transition-ghost cleanup for busy spans while bounding FFmpeg seek cost. `scripts/verify-face-track-scene-cut-guard.ps1` covers the budgeted branch as `directBudgetChecked=1` and `directBudgetSkipped=2`.
...
- To keep the broader scene-cut candidate check from stalling long or dense runs, the GUI and smoke paths only enable the expensive direct source-to-target frame comparison when the scene-cut candidate set is small (`<=24`). Larger candidate sets still use adjacent frame-difference scanning, weak-tail removal, and final-mask cleanup; the lower-level direct comparison remains covered by `scripts/verify-face-track-scene-cut-guard.ps1`.
