...
- Confirmed YOLO scene-cut pairs now generate a post-cut carry-window block list for the final stable gap-fill pass. This keeps direct post-cut YOLO detections, but prevents anti-flicker fill from recreating masks inside the immediate scene-transition carry window; `scripts/verify-yolo-final-mask-cleanup.ps1` reports `sceneCutCarryBlockedFrames=...`.
