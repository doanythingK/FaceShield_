...
- 2026-05-26 update: the final GUI gap-fill pass now passes both pre-smooth and post-smooth YOLO scene-cut pairs into `FillShortStableGaps`. If a known cut pair sits between the two anchors, the gap is not filled at all; this prevents the last anti-flicker pass from recreating a blur that the scene-cut guard already removed. `scripts/verify-yolo-final-mask-cleanup.ps1` covers this as `blockedCutGapFilled=0`.
...
- `scripts/verify-yolo-final-mask-cleanup.ps1` now also verifies blocked scene-cut gap fill with `blockedCutGapFilled=0`, proving known cut boundaries stop the final anti-flicker fill before it can recreate a post-transition blur.
