...
- YOLO initial backfill now also requires inward edge motion. A confirmed edge-start track whose second detection moves farther out toward the same edge is not backfilled into earlier frames. `scripts/verify-face-track-postprocess.ps1` covers both branches as `initialFilled=3` for inward edge-start fill and `outwardInitialFilled=False` for the outward edge-start block.
...
- `scripts/verify-face-track-postprocess.ps1` passed and now asserts `outwardInitialFilled=False`, proving YOLO initial backfill only repairs inward edge-start tracks and does not fill earlier frames for an edge track moving outward.
- `scripts/verify-yolo-profile-state.ps1` passed and now asserts `largeJumpGaps`/`largeJumpRanges` in both GUI and smoke final-mask summaries, plus `MaxInitialFillFrames`, `InitialFillRequiresInwardMotion`, initial-fill scene-cut candidates, initial-fill ROI refinement, `initialFilled` smoke logging, the `minConfidenceDrop=0.0` direct scene-cut carry check, the final stable-gap refill pass, and the direct-difference candidate cap.
