...
- Frames removed by YOLO final-mask cleanup are now reported separately when a later stable gap-fill pass skips them: `blockedByCleanup=...` and `cleanupBlockedFrames=...`. If any cleanup-blocked frame sits inside a stable anchor gap, the whole empty gap is blocked so the anti-flicker pass cannot recreate nearby parts of the same weak false-positive run. This gives direct run-log evidence that anti-flicker filling did not recreate a cleanup removal, and `frames=...` reports only newly filled frames rather than candidate/existing frames.
- 2026-05-26 smoke/evidence update: after `[SmokeFaceTrackSceneCutGuard]` and `[SmokeYoloFinalMaskPostSceneCleanup]`, the smoke harness now runs the same final stable-gap fill with `BlockedCutFramePairs = sceneCut.CutFramePairs`, cleanup removals in `BlockedFrameIndices`, and scene-carry windows in `BlockedSceneCarryFrameIndices`. It then logs `[SmokeYoloFinalMaskPostSceneGapFill]` and `[SmokeYoloFinalMaskPostSceneGapFillSceneCutGuard]`. The follow-up evidence writer prefers those post-scene lines when they exist, so short smoke evidence reflects the GUI's last YOLO postprocess stage instead of stopping at the earlier pre-scene gap fill.
...
- `scripts/verify-yolo-final-mask-cleanup.ps1` now also verifies cleanup-blocked gap fill with `cleanupBlockedGapFilled=0`, `cleanupBlockedGapFrames=1`, and `cleanupBlockedFrames=701`, plus whole-gap blocking with `partialCleanupBlockedGapFilled=0`, `partialCleanupBlockedGapFrames=3`, and `partialCleanupBlockedFrames=711,712,713`. This proves a frame removed by final-mask cleanup cannot be recreated by the stable anti-flicker fill pass, and nearby empty frames in the same suspect anchor gap are not partially recreated around it.
- `scripts/verify-yolo-final-mask-cleanup.ps1` now also verifies whole-gap scene-carry blocking with `partialSceneCarryRefillBlocked=3` and `partialSceneCarryBlockedFrames=3101,3102,3103`, proving one blocked transition-carry frame inside a stable anchor gap prevents partial refill around the carry window.
- 2026-05-27 scene-carry block extension: the direct high-confidence carry purge window remains 5 frames and covers same-geometry masks up to `0.98`, while matching weak carry masks at or below `0.78` are now removed through the 8-frame weak/block window. The final anti-flicker gap-fill block window is also 8 frames. `scripts/verify-yolo-final-mask-cleanup.ps1` covers this with `sceneCutCarryFrames=...` and extended `sceneCutCarryBlockedFrames=...`, so a confirmed transition can remove same-position residue and block later gap-fill recreation beyond the shorter high-confidence purge window without relying on confidence alone as proof.
...
- `scripts/verify-yolo-profile-state.ps1` now asserts that the smoke harness runs post-scene final gap fill with the scene-cut pairs and with cleanup-removed frames carried forward as blocked frames, and `scripts/verify-yolo-followup-quality-evidence.ps1` asserts that the evidence parser accepts `[SmokeYoloFinalMaskPostSceneGapFill]` and `[SmokeYoloFinalMaskPostSceneGapFillSceneCutGuard]`.
